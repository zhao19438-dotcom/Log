...
int main() {
    ...
    // 1. C 风格变参调用
    LOGI("欢迎使用 BitLog Plus！当前版本号: %s, 核心代号: %d", "v2.0.0", 2026);
    ...
    builder->buildSink<bitlog::FileSink>("./logs/sync.log");
    bitlog::Logger::ptr logger = builder->build();

    // 双前端交叉写入
    LOG_INFO(logger, "自定义同步日志器初始化完成，开始写入！");
    LOG_S_DEBUG(logger) << "正在扫描系统配置参数，耗时: " << 12.5 << "ms";
    LOG_S_WARN(logger)  << "连接池满载预警，当前活动连接: " << 1024;
    LOG_ERROR(logger, "数据库查询异常，错误码: %d, 详情: %s", 500, "Connection Timeout");
