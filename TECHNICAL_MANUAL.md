...
* **跨标准兼容分水岭**：以 C++11 为向下兼容基线，自研 `make_unique` 垫片平替 C++14，在 C++17 启用 `string_view` 与属性宏，向上探测支持 C++20 `std::source_location` 原生无宏直接调用。
...
本项目坚持**零外部三方依赖（Zero External Dependencies）**的设计原则，仅依赖标准库及操作系统基础接口，具备极佳的跨平台与跨标准移植能力。

* **支持标准**：C++11 / C++14 / C++17 / C++20 全标准兼容（基准测试推荐 C++17+）
* **支持平台**：
